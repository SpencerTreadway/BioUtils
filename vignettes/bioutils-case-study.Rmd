...
(GEO). The dataset, **GDS507**, contains expression profiles from 17 samples —
a mix of clear cell RCC tumor tissue and matched normal kidney tissue — measured
...
- **`geo$expression`** — a 22,645 × 17 matrix of log2-transformed probe
...
`get.gene.name()` performs the reverse lookup of `find.probe.by.gene()` —
given probe IDs from the TopTable, it retrieves the human-readable gene title
...
wide matrix (probes × samples) into a **long-format** data frame where each row
...
suppressor gene, which normally targets the HIF1α transcription factor for
degradation. Without VHL, HIF1α accumulates and drives a transcriptional
...
angiogenesis-related genes (driven by HIF1α pathway activation, common in RCC)
...
| `get.gene.name()` | Annotation df, probe IDs | Gene name strings | Probe → gene resolution |
| `find.probe.by.gene()` | Annotation df, gene names | Probe ID integers | Gene → probe resolution |
